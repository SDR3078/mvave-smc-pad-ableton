...
class MVave_SMC_PAD(ControlSurface):
    __doc__ = " Script for M-Vave SMC-PAD in APC emulation mode "
    ...
    def _combine_active_instances():
        track_offset = 0
        scene_offset = 0
        for instance in MVave_SMC_PAD._active_instances:
    ...
    def __init__(self, c_instance):
        ...
        with self.component_guard():
            ...
            self._mixer = None
            self._missing_clip_setters = []
            self._modifier_button = None
    ...
    def disconnect(self):
        if self._modifier_button is not None:
            self._modifier_button.remove_value_listener(self._modifier_value)
            self._modifier_button = None
        self._note_map = None
        self._ctrl_map = None
        self._pads = None
    ...
    def _do_combine(self):
        if self not in MVave_SMC_PAD._active_instances:
            MVave_SMC_PAD._active_instances.append(self)
            MVave_SMC_PAD._combine_active_instances()
    ...
    def _setup_session_control(self):
        ...
        for scene_index in range(TSB_Y):    # Change range() value to set the vertical count for track selection box
            ...
            for track_index in range(TSB_X):    # Change range() value to set the horizontal count for track selection box
                ...
                self._apply_clip_colours(clip_slot)
        ...
        self._setup_modifier()

    def _apply_clip_colours(self, clip_slot):
        # Set by name and guarded on purpose: _Framework's ClipSlotComponent
        # has gained and lost these setters across Live versions, and a missing
        # one must not take the script down -- clip launching still works
        # without the colour. Logged once rather than sixteen times.
    ...
    def _setup_modifier(self):
        # Note 19 gives up its transport role so it can act as a plain
        # modifier. Nothing is shared with the transport component: sharing a
        # button would mean guessing at _Framework internals that are not on
        # the machine this was written on.
        self._modifier_button = self._note_map[MODIFIER]
        if self._modifier_button is not None:
            self._modifier_button.add_value_listener(self._modifier_value)
            self.log_message('MVave_SMC_PAD: modifier on note %d' % MODIFIER)

    def _modifier_value(self, value):
        self._set_session_banking(scenes=(value != 0))

    def _set_session_banking(self, scenes):
        # The same two buttons, swapped between the session component's two
        # banking roles. Passing None to unhook is the template's own idiom
        # for an unassigned button, so it is a supported value here.
